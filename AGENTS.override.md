# agent-rules Engineering Codex Guidance
...
- Delegate through provider agents in `.codex/agents` using this baseline order: `plan-agent -> approval -> coach-agent -> build-agent/debug-agent -> verify -> handoff`.
...
- Invoke `build-agent` for implementation/refactor work and `debug-agent` for failure diagnosis/fixes.
...
- Use strict TypeScript with explicit contracts at every module boundary.
- Enable `strict` mode and prefer `noUncheckedIndexedAccess` / `exactOptionalPropertyTypes` where feasible.
- Avoid `any`; prefer `unknown` plus explicit narrowing and exhaustive checks.
- Prefer discriminated unions and branded types for domain identifiers over ad-hoc primitives.
- Avoid default exports; prefer explicit named exports and imports.
- Prefer functional approaches and composition over class-based OOP.
- Prefer named exports and composition over large class-heavy inheritance trees.
- Prefer immutable defaults (`readonly`, `ReadonlyArray`) for shared data structures.
- Prefer `async`/`await` over raw `Promise` chaining for readability and stack traces.
- Validate untrusted runtime input with schemas before treating it as trusted typed data.
- Favor small pure utilities and typed adapters over broad mutable service objects.
...
- Delegate first to `plan-agent` and wait for explicit approval.
- Delegate second to `coach-agent`; only continue when coach returns GO.
...
### Debugging
- Delegate first to `plan-agent` with failing surface and intended approach.
- After approval, run `coach-agent` to check risk and test gaps.
...
- Route through `plan-agent` and `coach-agent` before `build-agent` execution.
...
- Confirm baseline handoff order `plan-agent -> approval -> coach-agent -> build-agent/debug-agent -> verify -> handoff` in responses.
- Confirm `Approval level` and required approval checkpoints are explicit in plans/responses.
